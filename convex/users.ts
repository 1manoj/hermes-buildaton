...
export const setRole=mutation({args:{userId:v.id("users"),role:v.string()},handler:async(ctx,args)=>ctx.db.patch(args.userId,{role:args.role})});
export const get=query({args:{userId:v.id("users")},handler:async(ctx,args)=>ctx.db.get(args.userId)});
